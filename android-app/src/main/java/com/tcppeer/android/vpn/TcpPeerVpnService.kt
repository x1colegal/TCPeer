...
class TcpPeerVpnService : VpnService() {
    ...
    private fun connect() {
        ...
        connectionJob = serviceScope.launch {
            try {
                ...
                closeResources()
                val shouldRestart = restartRequested.getAndSet(false) && !disconnectRequested.get()
                if (shouldRestart) {
                    connectionJob = null
                    serviceScope.launch {
                        delay(750)
                        if (!disconnectRequested.get()) connect()
                    }
                } else if (disconnectRequested.get()) {
                    TcpPeerRuntime.replace(VpnRuntimeState())
    ...
    private suspend fun runConnection(config: VpnConfiguration) = withContext(Dispatchers.IO) {
        ...
        val address = InetAddress.getByName(punch.field("Address") ?: throw ProtocolException("PUNCH-GO has no address"))
        if (family == DirectFamily.IPV6 && address !is Inet6Address) throw ProtocolException("TCP6 requires an IPv6 endpoint")
        if (family == DirectFamily.IPV4 && address !is Inet4Address) throw ProtocolException("TCP4 requires an IPv4 endpoint")
        if (family == DirectFamily.IPV4 && localIpv6.isNotEmpty() && TransportPolicy.isUsableIpv6(address)) {
        ...
        if (waitMillis > 0) kotlinx.coroutines.delay(waitMillis)
        val peerPort = punch.field("Port")?.toIntOrNull() ?: throw ProtocolException("PUNCH-GO has no valid port")
        updateConnecting("Opening a direct ${family.name.replace("IPV", "TCP")} connection.")
        val direct = openDirect(address, peerPort, config.directPort, family).also { directSocket = it }
    ...
    private suspend fun exchangePackets(
        ...
        val peerToTun = launch(Dispatchers.IO) {
            while (true) {
                ...
                if (tpp?.type == TppProtocol.ECHO_REPLY) {
