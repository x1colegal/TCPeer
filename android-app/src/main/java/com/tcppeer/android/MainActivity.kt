        ...
        if (connected) {
            ...
            if (runtime.devices.isEmpty()) {
                ...
                items(runtime.devices.size) { index ->
                    ...
                    DeviceCard(device) { TcpPeerRuntime.startContinuousPing(device.peerId, device.overlayIpv6) }
                }
            }
            item {
...
@Composable
private fun DeviceCard(device: NetworkDevice, onPing: () -> Unit) {
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Column {
                        ...
                        Text(
                            listOf(device.overlayIpv6, device.transport).filter { it != "-" }.joinToString(" - "),
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                        )
                    }
